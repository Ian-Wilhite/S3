...
Image::Image(const Image &old_image) : width(old_image.width), height(old_image.height) {
    if (width > 0 && height > 0) {
        image = new Pixel*[width];
        for (unsigned int col = 0; col < width; col++) {
            image[col] = new Pixel[height];
            for (unsigned int row = 0; row < height; row++) {
                image[col][row] = old_image.image[col][row];
            }
        }
    } else {
        image = nullptr;
...
Image& Image::operator=(const Image &old_image) {
    if (this != &old_image) {  // Prevent self-assignment
...
Image::~Image() {
    clear(); // Reuse the clear function to free allocated memory
}
